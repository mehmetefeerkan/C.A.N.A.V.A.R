...
let master_ = "alternative-will-tragedy-congo.trycloudflare.com"
let master = null
...
function checkMaster(master__) {
    ...
    axios.get("http://" + master__ + "/status")
        .then(res => {
...
settingIntegrity.on('true', () => {
    ...
    app.get('/installScript/:scriptid', (req, res) => { //OPTIMIZE THIS!
        let scriptid = req.params.scriptid
        axios.get("http://" + master + "/scripts?id=" + scriptid)
            .then(resp => {
                console.log(resp.data)
                axios.get("http://" + master + "/scripts?id=" + scriptid + "&download")
                    .then(resp => {
